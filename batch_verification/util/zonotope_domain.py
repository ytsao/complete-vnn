...
class ZonotopeDomain(AbstractDomain):
    ...
    @classmethod
    def with_center_and_generators(
        cls, _center: float, _var_generators: List[str], _coef_generators: List[float]
    ):
    ...
    def add(self, other: Any) -> Any:
        ...
        self.coef_generators = (
            jnp.array(self.coef_generators) + jnp.array(other.coef_generators)
        ).tolist()
        # * self.var_generators remains the same
        self.concrete_domain.lower_bound = self.center - jnp.sum(
            jnp.abs(jnp.array(self.coef_generators))
        )
        self.concrete_domain.upper_bound = self.center + jnp.sum(
            jnp.abs(jnp.array(self.coef_generators))
        )
    ...
    def substract(self, other: Any) -> Any:
        ...
        self.concrete_domain.uppper_bound = self.center + jnp.sum(
            jnp.abs(jnp.array(self.coef_generators))
        )
        self.lower_bound = self.concrete_domain.lower_bound
        self.upper_bound = self.concrete_domain.upper_bound
    ...
    def join(self, other: Any) -> Any:
        ...
        for x, y in zip(self.coef_generators, other.coef_generators):
            ...
            is_included_zero: bool = (
                True if min_value <= 0 and max_value >= 0 else False
            )
            new_coef: float = (
                0 if is_included_zero else min(abs(min_value), abs(max_value))
            )
...
if __name__ == "__main__":
    ...
    print(
        "zonotope_domain1: ",
        zonotope_domain1.lower_bound,
        zonotope_domain1.upper_bound,
        zonotope_domain1.center,
    )
    print(
        "zonotope_domain2: ",
        zonotope_domain2.lower_bound,
        zonotope_domain2.upper_bound,
        zonotope_domain2.center,
    )
    print("---------------------------------------------------------------")
    print(
        "zonotope concrete domain1: ",
        zonotope_domain1.concrete_domain.lower_bound,
        zonotope_domain1.concrete_domain.upper_bound,
    )
    print(
        "zonotope concrete domain2: ",
        zonotope_domain2.concrete_domain.lower_bound,
    ...
    print(
        "zonotope_domain1 after transformer: ",
        zonotope_domain1.lower_bound,
        zonotope_domain1.upper_bound,
        zonotope_domain1.center,
    )
    ...
    print(
        "zonotope_domain2 after transformer: ",
        zonotope_domain2.lower_bound,
        zonotope_domain2.upper_bound,
        zonotope_domain2.center,
    )
    ...
    print(
        "zonotope_domain3 after join: ",
        zonotope_domain3.lower_bound,
        zonotope_domain3.upper_bound,
        zonotope_domain3.center,
    )
