...
class _Constraints():
    ...
    def post_condition(m: SCIPModel | GurobiModel, networks: NetworksStructure) -> SCIPModel | GurobiModel:
        ...
        if num_post_condition > 1:
            expression = sum(v_var for k_var, v_var in m.solver.binary_variables.items(
            ) if "y_pos" in k_var) >= 1
            ...
            for i in range(num_post_condition):
                for j in range(len(networks.post_condition[i][0])):
                    ...
                    if rhs > 0:
                        lhs = sum(networks.post_condition[i][0][j][k] * m.solver.continue_variables[f"x_{last_layer_id}_{k}"] for k in range(
                            networks.layer_to_layer[-1][1]))
                        m.add_constraint(
                            express=lhs >= -rhs * m.solver.binary_variables[f"y_pos_{i}"], name=f"post_condition2_{i}_{j}")
                        m.add_constraint(express=lhs <= rhs * (
                            1 - m.solver.binary_variables[f"y_pos_{i}"]), name=f"post_condition2_{i}_{j}")
                        ...
                        lhs = sum(networks.post_condition[i][0][j][k] * m.solver.continue_variables[f"x_{last_layer_id}_{k}"] for k in range(
                            networks.layer_to_layer[-1][1]))
                        m.add_constraint(
                            express=lhs >= -9999 * m.solver.binary_variables[f"y_pos_{i}"], name=f"post_condition2_{i}_{j}")
                        m.add_constraint(express=lhs <= 9999 * (
                            1 - m.solver.binary_variables[f"y_pos_{i}"]), name=f"post_condition2_{i}_{j}")
            ...
            for j in range(len(networks.post_condition[0][0])):
                lhs = sum(networks.post_condition[0][0][j][k] * (m.solver.continue_variables[f"x_{last_layer_id}_{k}"] -
                          m.solver.continue_variables[f"s_{last_layer_id}_{k}"]) for k in range(networks.layer_to_layer[-1][1]))
    ...
    def relu(m: SCIPModel | GurobiModel, networks: NetworksStructure) -> SCIPModel | GurobiModel:
        for k, Nk in enumerate(networks.layer_to_layer):
            ...
            for nk in range(Nk[0]):
                ...
                expression = m.solver.continue_variables[f"s_{k}_{nk}"] <= 9999 * \
                    m.solver.binary_variables[f"z_{k}_{nk}"]
            ...
            if k == networks.num_layers - 2:
                for nk in range(Nk[1]):
                    expression = m.solver.continue_variables[f"x_{k+1}_{nk}"] <= 9999 * (
                        1 - m.solver.binary_variables[f"z_{k+1}_{nk}"])
                    m.add_constraint(express=expression,
                                     name=f"relu_{k+1}_{nk}_1")

                    expression = m.solver.continue_variables[f"s_{k+1}_{nk}"] <= 9999 * \
