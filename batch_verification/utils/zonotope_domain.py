...
class ZonotopeDomain(AbstractDomain):
    ...
    def __init__(self,
        ...
        self.concrete_domain: BoxDomain = BoxDomain(
            _lower_bound=_lower_bound, _upper_bound=_upper_bound)
    ...
    def with_center_and_generators(cls, _center: float, _var_generators: List[str], _coef_generators: List[float]):
        if len(_var_generators) != len(_coef_generators):
            raise ValueError(
                "the number of variables and coefficients should be the same.")
    ...
    def add(self, other: Any) -> Any:
        if len(self.var_generators) == 0 or len(other.var_generators) == 0:
            raise ValueError(
                "There is no generator variables in zonotope domain.")

        self.center += other.center
        self.coef_generators = (
            jnp.array(self.coef_generators) + jnp.array(other.coef_generators)).tolist()
        # * self.var_generators remains the same
        self.concrete_domain.lower_bound = self.center - \
            jnp.sum(jnp.abs(jnp.array(self.coef_generators)))
        self.concrete_domain.upper_bound = self.center + \
            jnp.sum(jnp.abs(jnp.array(self.coef_generators)))
    ...
    def substract(self, other: Any) -> Any:
        ...
        self.concrete_domain.lower_bound = self.center - \
            jnp.sum(jnp.abs(jnp.array(self.coef_generators)))
        self.concrete_domain.uppper_bound = self.center + \
            jnp.sum(jnp.abs(jnp.array(self.coef_generators)))
        self.lower_bound = self.concrete_domain.lower_bound
        self.upper_bound = self.concrete_domain.upper_bound
    ...
    def join(self, other: Any) -> Any:
        ...
        for x, y in zip(self.coef_generators, other.coef_generators):
            ...
            new_coef: float = 0 if is_included_zero else min(
        ...
        new_coef: float = temp_box.upper_bound - result.center - \
            float(jnp.sum(vmap(jnp.abs)(jnp.array(result.coef_generators)))
                  )   # * jax version
        result.coef_generators.append(new_coef)
...
if __name__ == "__main__":
    # * test ZonotopeDomain
    zonotope_domain1: ZonotopeDomain = ZonotopeDomain.from_lower_and_upper_bound(
        _lower_bound=-5.0, _upper_bound=1.0)
    zonotope_domain2: ZonotopeDomain = ZonotopeDomain.from_lower_and_upper_bound(
        _lower_bound=1.0, _upper_bound=2.0)

    print("zonotope_domain1: ", zonotope_domain1.lower_bound,
          zonotope_domain1.upper_bound, zonotope_domain1.center)
    print("zonotope_domain2: ", zonotope_domain2.lower_bound,
          zonotope_domain2.upper_bound, zonotope_domain2.center)
    print("---------------------------------------------------------------")
    print("zonotope concrete domain1: ", zonotope_domain1.concrete_domain.lower_bound,
          zonotope_domain1.concrete_domain.upper_bound)
    print("zonotope concrete domain2: ", zonotope_domain2.concrete_domain.lower_bound,
          zonotope_domain2.concrete_domain.upper_bound)
    print("---------------------------------------------------------------")

    zonotope_domain1.abstract_transformer()
    print("zonotope_domain1 after transformer: ", zonotope_domain1.lower_bound,
          zonotope_domain1.upper_bound, zonotope_domain1.center)
    ...
    print("zonotope_domain2 after transformer: ", zonotope_domain2.lower_bound,
          zonotope_domain2.upper_bound, zonotope_domain2.center)
    ...
    print("zonotope_domain3 after join: ", zonotope_domain3.lower_bound,
          zonotope_domain3.upper_bound, zonotope_domain3.center)
